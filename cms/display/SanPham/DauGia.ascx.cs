...
public partial class cms_display_SanPham_DauGia : System.Web.UI.UserControl
{
    ...
    {
        DataTable dt = HiPhucShop.PhienDauGia.Thongtin_PhienDauGia();
        if (dt.Rows.Count>0)
        {
    ...
    private string LayThongTinSanPham(string MaSP)
    {
        ...
        DataTable dt = HiPhucShop.SanPham.Thongtin_Sanpham_by_id(MaSP);
        if (dt.Rows.Count>0)
        {
            ...
            if (giaDaChon%1000 == 0)
            {
                ...
                if (TongLuotDauGia()<10)
                {
                    ...
                    if (ConThoiGianDauGia())
                    {
                        ...
                        string maXacNhan = HiPhucShop.MaHoa.MaHoaMD5(hdMaKH.Value + hdMaPhienDauGia.Value + DateTime.Now.Ticks);

                        HiPhucShop.LuotDauGia.LuotDauGia_Inser(DateTime.Now.ToString(), tbGiaBanChon.Text, maXacNhan, hdMaKH.Value, hdMaPhienDauGia.Value, "");
...
<div style='font:normal 12px Arial;padding-bottom:15px'>
    ...
    {
        DataTable dt = HiPhucShop.LuotDauGia.Thongtin_LuotDauGia_by_makh_maphiendg(hdMaKH.Value, hdMaPhienDauGia.Value);
